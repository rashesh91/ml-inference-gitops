...
async def run_agent(
    ...
    for iteration in range(settings.agent_max_iterations):
        ...
        # Check for TOOL: call
        tool_match = re.search(r"TOOL:\s*(\{.*?\})", response_text, re.DOTALL)
        ...
        if tool_match:
            ...
            if on_event:
                on_event(AgentEvent("tool_call", {"tool": tool_name, "args": tool_args}))

            result = await _execute_tool(tool_name, tool_args)
            log.info(f"Tool {tool_name}({tool_args}) → {result[:80]}")

            if on_event:
                on_event(AgentEvent("tool_result", {"tool": tool_name, "result": result}))
        ...
        if answer_match:
            ...
            if on_event:
                on_event(AgentEvent("answer", {"text": answer}))
            return answer
        ...
        if on_event:
            on_event(AgentEvent("answer", {"text": response_text}))
        return response_text

    # Exhausted iterations
    fallback = "I'm sorry, I couldn't complete that request in time."
    if on_event:
        on_event(AgentEvent("error", {"text": fallback}))
    return fallback
