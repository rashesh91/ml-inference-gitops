#!/usr/bin/env python3
"""
Sanjana FreeSWITCH AI Bridge — v4 (India-optimised)
-----------------------------------------------------
New in v4:
  - Sarvam AI STT (saarika:v2) + TTS (bulbul:v1)  — best for Indian languages
  - Per-turn latency instrumentation  (LATENCY log line per turn)
  - Call summary on hangup            (CALL_SUMMARY log line)
  - RECORD_SILENCE_SEC reduced to 1s  (saves 1s perceived latency)
  - Sarvam TTS returns 8kHz WAV directly — no ffmpeg for TTS path

Provider selection (env vars):
  STT_PROVIDER = sarvam | whisper   (default: sarvam)
  TTS_PROVIDER = sarvam | gtts      (default: sarvam)
  SARVAM_API_KEY = <your key>       (get at dashboard.sarvam.ai)

DTMF language menu:
  1 = Hindi (hi-IN, speaker meera)
  2 = English (en-IN, speaker meera)
  3 = Gujarati (gu-IN, speaker meera)
  (no digit → English)

Latency log format:
  LATENCY session=X turn=N lang=hi silence_ms=820 stt_ms=248 llm_ms=1034
          tts_ms=195 ffmpeg_ms=0 total_ms=2297
  CALL_SUMMARY session=X turns=6 avg_ms=2290 min_ms=1980 max_ms=2870
               lang=hi hangup=NORMAL_CLEARING
"""
import base64
import logging
import os
...
class CallStats:
    ...
    def log_summary(self, session_id: str, lang: str, hangup: str = ""):
        if not self._totals:
            return
...
def handle_call(sock: socket.socket, addr):
    ...
    while True:
        ...
        # 3. LLM + TTS
        log.info(f"Turn {turn}: LLM+TTS…")
        mp3_bytes = _chat(transcript, session_id, lang["lang_code"], lang["sarvam_spk"])
        timer.mark("llm")

        if mp3_bytes is None:
            log.warning(f"Turn {turn}: gateway failed — fallback")
            _play_wav(sock, call_uuid, FALLBACK_WAV, ev_queue, timeout=10)
            continue

        # 4. Write audio — Sarvam returns WAV already; gTTS returns MP3
        ...
        # 5. Play with barge-in
        log.info(f"Turn {turn}: playing response")
        _play_wav(sock, call_uuid, wav_path, ev_queue, timeout=30)
        _cleanup(wav_path)

        total = timer.total_ms()
        timer.log(session_id, turn, lang["lang_code"])
        stats.add(total)

    stats.log_summary(session_id, lang["lang_code"], hangup_c)
    stop_reader.set()
    sock.close()
